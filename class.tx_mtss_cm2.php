...
class tx_mtss_cm2 {
	...
	function main(&$backRef,$menuItems,$table,$uid)	{
		...
		if (!$backRef->cmLevel)	{
				...
				$url = t3lib_extMgm::extRelPath('mtss').'cm2/index.php?id='.$uid;

				$localItems[] = $backRef->linkItem(

					$GLOBALS["LANG"]->getLLL("cm2_title",$LL),

					$backRef->excludeIcon('<img src="'.t3lib_extMgm::extRelPath("mtss").'cm2/cm_icon.gif" width="15" height="12" border="0" align="top" />'),
			...
			if (@is_file($table)) {
				...
				if ($BE_USER->isAdmin() || stristr($BE_USER->groupData['modules'],'web_txmtssM1')) {
					...
					if ($enabled) {
						...
						$url = t3lib_extMgm::extRelPath('mtss').'cm2/index.php?file='.rawurlencode($table);

						$url='mod.php?&M=web_txmtssM1&SET[function]=2&FCE=1&file='.rawurlencode($table);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/mtss/class.tx_mtss_cm2.php'])	{

	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/mtss/class.tx_mtss_cm2.php']);
